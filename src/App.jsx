...
/** Generate pixel data export string */
function generateExport(pixels, cols, rows, language, format) {
  const bytes = [];
  // Pack pixels into bytes (MSB first), row by row
...
export default function App() {
  ...
  // Track current pixels in a ref so stroke-start snapshot is always fresh
  const pixelsRef = useRef(pixels);
  pixelsRef.current = pixels;

  // Frames (Phase 1: single frame, structure ready for multi-frame)
